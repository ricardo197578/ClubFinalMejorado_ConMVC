...
using System.Linq; // ¡Aquí se llama a LINQ!
...
namespace ClubMinimal.Repositories
{
    public class NoSocioRepository : INoSocioRepository
    {
        ...
        {
            if (noSocio == null)
                throw new ArgumentNullException("noSocio"); // Cambiado aquí

            if (string.IsNullOrWhiteSpace(noSocio.Dni))
                throw new ArgumentException("El DNI es requerido", "Dni"); // Cambiado aquí

            var sql = @"INSERT INTO NoSocios 
                (Nombre, Apellido, Dni, FechaRegistro) 
        ...
        {
            // para buscar en tu base de datos
